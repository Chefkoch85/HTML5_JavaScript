...
// -> size and mouse-pos
var gSize = new Point(0,0);
var gPointMouse = new Point(0,0);
// -> ui vars
var gbtnStart;
...
function DrawPlayerInfo(pInfo, s, f)
{    
    pInfo.textContent = "S:P/L/T " + s.Pos.x + ":" + s.Pos.y + 
			" / " + s.parts.length + " / " + s.timer.time.toFixed(3) +
			" | F:P " + f.pos.x + ":" + f.pos.y;
}
...
var gTimer = new Timer(2);
// -> game vars
var snake = new Snake(new Point(10,5), new Point(10,10), "rgb(255,150,50)");
var fruit = new Fruit(new Point(0,0), new Point(10,10), "rgb(50,200,100)");
...
function InitGameVars()
{
    ...
    gColLerp.Reset();
    fruit.Init();
    snake.Init(new Point(Math.floor((gSize.x / 2) / 10), 
			 Math.floor((gSize.y / 2) / 10)));
...
function Loop()
{
    ...
    DrawPlayerInfo(gpInfo2, snake, fruit);
...
function Update(gt)
{
    ...
    var s = new Point(Math.floor(gSize.x / 10), 
		      Math.floor(gSize.y / 10));
		      
    if(snake.HitBorder(s) || snake.HitSelf())
    {
	alert("game over, point= " + snake.parts.length);
	StartGame();
    }
    
    //console.log("SP: " + snake.Pos.x + ":" + snake.Pos.y +
    //		" - FP: " + fruit.pos.x + ":" + fruit.pos.y);
    if(fruit.Hit(snake))
    {
